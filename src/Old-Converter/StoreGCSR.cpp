...
void GridCSRConverter::storeGCSR(GridCSR::FS::path const & folderPath, std::string const & dataName) {
    GridCSR::MetaData m;
    ...
    GridCSR::FS::create_directory(rootFolder);

    m.Save(rootFolder.string() + std::string("meta.json"));
    ...
    tbb::parallel_for_each(m.grid.each.begin(), m.grid.each.end(),
        [this, &m, &rootFolder](decltype(m.grid.each.front()) const & i){
            ...
            of.write((char*)o.data(), sizeof(GridCSR::Vertex) * o.size());
            of.close();
        });
