...
func reorderSliceToByteSlice(in []reorder) []byte {
...
func reorderSliceToUint64Slice(in []reorder) []uint64 {
	header := *(*reflect.SliceHeader)(unsafe.Pointer(&in))
	header.Len = len(in) * 2
	header.Cap = len(in) * 2
...
func mapperB(adj6 []uint8, globalSlice []reorder, wg *sync.WaitGroup, in <-chan sRawDat) {
	wg.Add(1)
	go func() {
		defer wg.Done()
...
func subroutineB(rSlice *[]reorder) {
	files := walk()
...
func subroutineC(rSlice *[]reorder) {
	...
	default:
		log.Fatalln("reorder Type is wrong")
	}
...
func routine() {
	...
	stopwatch("Allocate memory space for reordering map", func() {
	...
	stopwatch("subroutineC (make reordering map)", func() {
		subroutineC(&rSlice)
	})
	...
	*/
	...
	stopwatch("subroutineD (write reordering map to file; uint64 little endian)", func() {
		subroutineD(&rSlice)
	})
}
...
func init() {
	...
	reorderType := flag.Int("reorder.type", -1, "Output File")
	flag.Parse()

	if len(*inFolder) == 0 || len(*outFile) == 0 || *reorderType == -1 {
		flag.Usage()
		os.Exit(1)
	}
	...
	*outFile, _ = filepath.Abs(*outFile)
	...
	ctx = context.WithValue(ctx, "reorderType", *reorderType)
}
