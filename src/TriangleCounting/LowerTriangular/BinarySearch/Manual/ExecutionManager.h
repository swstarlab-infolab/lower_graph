    ...
    std::shared_ptr<bchan<CommandResult>>& out)
{
    for (auto& req : *in.get()) {
        ...
        std::array<MemoryInfo, 3> memoryInfo;
        std::array<fiber, 3> waitGroup;
        for (int i = 0; i < 3; i++) {
            waitGroup[i] = fiber([&, myID, i] {
                // query
                bchan<MemoryInfo> tempChan(1);
                auto a = ctx.memChan[myID];
                a.push(
                    DataRequest { Method::Query, req.gidx[i], tempChan });
        ...
        res.triangles = distTriangle(mersenneTwister64);
        res.elapsedTime = std::chrono::duration<double>(end - start).count();

        out.get()->push(res);
        // prepare random machine
    }
    ...
    std::shared_ptr<bchan<Command>>& in)
{
    ...
    // prepare channels
    if (myID > 0) {
        std::thread([&, myID] { ComputationGPU(ctx, myID, in, out); }).detach();
    } else {
        std::thread([&, myID] { ComputationCPU(ctx, myID, in, out); }).detach();
    }
    ...
    int gpuID)
{
    ...
    buddy.init(memrgn_t { (void*)myMemory.first.get(), myMemory.second }, 256, 1);
    ...
    for (auto& cReq : cmdReq) {
        ...
        if (searchInMyMap(cReq.G, location)) {
            fprintf(stdout, "[GPU%d  ] Find Memory: (%ld,%ld)(%ld,%ld)(%ld,%ld)\n", gpuID,
                cReq.G[0][0], cReq.G[0][0],
                cReq.G[1][0], cReq.G[1][0],
                cReq.G[2][0], cReq.G[2][0]);
        } else {
            fprintf(stdout, "[GPU%d  ] Not Find Memory: (%ld,%ld)(%ld,%ld)(%ld,%ld), Request to Load Manager\n", gpuID,
                cReq.G[0][0], cReq.G[0][0],
                cReq.G[1][0], cReq.G[1][0],
                cReq.G[2][0], cReq.G[2][0]);
            ...
            fprintf(stdout, "[GPU%d  ] Not Find Memory: (%ld,%ld)(%ld,%ld)(%ld,%ld), Waiting Load Manager Response\n", gpuID,
                cReq.G[0][0], cReq.G[0][0],
                cReq.G[1][0], cReq.G[1][0],
                cReq.G[2][0], cReq.G[2][0]);
            ...
            for (unsigned int g = 0; g < cReq.G.size(); g++) {
                ...
                for (unsigned int i = 0; i < ptr.size(); i++) {
                    ...
                    cudaMemcpy(a->second.arr[i].first, lRes.arr[i].first, lRes.arr[i].second, cudaMemcpyDeviceToHost);
