...
using GridIndex = std::string;
using Command	= std::array<GridIndex, 3>;
...
struct KeyHash {
	...
	{
		auto a = std::hash<std::string>{}(k.idx);
		auto b = std::hash<uint64_t>{}(k.type);
		return a ^ b;
	}
};
...
struct KeyEqual {
	...
	{
		return (kl.idx == kr.idx && kl.type == kr.type);
	}
};
