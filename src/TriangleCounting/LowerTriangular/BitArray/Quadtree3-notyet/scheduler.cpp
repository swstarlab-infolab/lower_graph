...
void Scheduler::init(fs::path const & folderPath)
{
		...
		for (size_t col = 0; col <= row; col++) {
			for (size_t i = col; i <= row; i++) {
				...
				boost::asio::post(pool, [=] {
					...
					for (int j = 0; j < 3; j++) {
						size_t exist = mysqlSingleValueSizeT(myConn,
															 sprn("\
							SELECT COUNT(*)\
							FROM grids AS g\
	...
	LOGF("CRITERIA=%ld bytes", this->criteria);
}

Scheduler::Grid3 Scheduler::fetchJob(int const device_id)
{
	mysqlpp::Connection conn;
	mysqlConnect(conn);
