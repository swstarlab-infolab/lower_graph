...
func mapper(adj6 []uint8, relabelFile []uint64, in <-chan sRawDat) <-chan []gridEdge {
	...
	go func() {
		...
		for dat := range in {
			...
			for i := uint64(0); i < dat.cnt; i++ {
				s := relabelFile[dat.src]
				now := dat.dstStart + i*wordByte
				d := relabelFile[convBE6toLE8(adj6[now:now+wordByte])]
...
func routine() {
	...
	relabelFile := byteSliceToUint64Slice(loader(ctx.Value("relabelFile").(string)))
		...
		go func(in <-chan string) {
			...
			for file := range in {
				...
				for i := 0; i < shufflers; i++ {
					mapped := mapper(data, relabelFile, splitPos)
...
func init() {
	...
	relabelFile := flag.String("relabel.file", "", "Reorder table file")
	outFolder := flag.String("out.folder", "", "Output Folder")
	outName := flag.String("out.name", "", "Output Name")
	flag.Parse()

	if len(*inFolder) == 0 || len(*outFolder) == 0 || len(*outName) == 0 || len(*relabelFile) == 0 {
		flag.Usage()
		os.Exit(1)
	}

	ctx = context.WithValue(ctx, "inFolder", *inFolder)
	ctx = context.WithValue(ctx, "relabelFile", *relabelFile)
