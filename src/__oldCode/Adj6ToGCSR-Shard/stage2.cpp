...
{
	// init
	auto out = makeSp<std::valarray<E32>>(in->size());

	std::valarray<std::atomic<uint32_t>> bitvec(size_t(ceil(double(in->size()) / 32.0)));
	...
	std::valarray<uint64_t> pSumRes(in->size() + 1);
		...
		[&](tbb::blocked_range<size_t> const & r) {
			for (size_t grain = r.begin(); grain != r.end(); grain += r.grainsize()) {
				for (size_t offset = 0; offset < r.grainsize(); offset++) {
					...
					if (getBit(i)) {
						// try {
						out->operator[](pSumRes[i]) = in->at(i);
...
static void writeCSR(fs::path const outTarget, std::valarray<E32> const & in)
{
			...
			[&](tbb::blocked_range<size_t> const & r) {
				...
				for (size_t grain = r.begin(); grain != r.end(); grain += r.grainsize()) {
					for (size_t offset = 0; offset < r.grainsize(); offset++) {
						...
						auto curr = grain + offset;
						if (in.operator[](prev)[0] != in.operator[](curr)[0]) {
							setBit(curr);
							myOnes++;
						}
					}
	...
	parallelDo(8, [&](size_t const i) {
		for (auto & fPath : *jobs) {
			stopwatch("Stage2, " + std::string(fPath), [&] {
				...
				size_t shard  = 1L << 28;
				size_t shards = ceil(deduped->size(), shard);

				parallelDo(shards, [&](size_t const s) {
					fs::path target;
					auto &	 d = *deduped;

					if (shards <= 1) {
						target = fPath.parent_path() / fPath.stem();
					} else {
						target = fs::path(std::string(fPath.parent_path() / fPath.stem()) + "," +
										  std::to_string(s));
					}

					if (s != shards - 1) {
						writeCSR(target, d[std::slice(s * shard, shard, 1)]);
					} else {
						writeCSR(target,
								 d[std::slice(s * shard, deduped->size() - (s * shard), 1)]);
					}
				});
