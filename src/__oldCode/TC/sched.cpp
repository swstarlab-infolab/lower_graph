	...
	for (fs::recursive_directory_iterator curr(directory), end; curr != end; ++curr) {
		if (fs::is_regular_file(curr->path()) && fs::file_size(curr->path()) > 0) {
			...
			if (sIdx.parse(curr->path().stem().string()) && curr->path().extension() == extension) {
				// printf(" matched: (%d,%d),%d,(%d,%d)", sIdx.grid[0], sIdx.grid[1],
				// sIdx.depth, sIdx.shard[0], sIdx.shard[1]);
			}
			// printf("\n");
...
sp<bchan<Job>> Manager::run(sp<Context> ctx)
{
	auto out = makeSp<bchan<Job>>(16);

	std::thread([=] {
		auto const MAXROW = getGridSize(ctx->inFolder, ctx->extension[0]);
		// if (ctx.verbose) {
		// printf("MAXROW: %d\n", MAXROW);
		//}
			...
			for (uint32_t col = 0; col <= row; col++) {
				for (uint32_t i = col; i <= row; i++) {
					...
					auto findShards = [&ctx](XY const & idx) {
						...
						for (fs::recursive_directory_iterator curr(ctx->inFolder), end; curr != end;
							 ++curr) {
							if (fs::is_regular_file(curr->path()) &&
								fs::file_size(curr->path()) > 0 &&
								curr->path().extension() == ctx->extension[0]) {
								...
								if (checkAvail(std::array<ShardIndex, 3>{A, B, C})) {
									...
									printf("-(%d,%d),%d,(%d,%d)\n",
										   ...
										   C.shard[1]);
																	...
																	auto ARange =
									   convShardIndexToRange(A); auto BRange =
									convShardIndexToRange(B); auto CRange =
									   convShardIndexToRange(C); increaseRangeDepth(ARange, 2);
									...
									out->push(Job{A.string(), B.string(), C.string()});
								}
							}
						}
					}
